...
export function IdUploadPanel({
  ...
  onContinue: () => void;
  /** Mobile full-screen flow only: stretches to the slide's full height and
   *  pins Continue to the bottom (thumb reach) instead of it trailing right
   *  under the dropzone with dead space below. Desktop doesn't pass this —
   *  its card-embedded layout is unaffected. */
  fill?: boolean;
}) {
  ...
  return (
    <div className={clsx("max-w-[640px] mx-auto w-full", fill && "h-full flex flex-col")}>
      <div className={clsx(fill && "flex-1 flex flex-col justify-center")}>
      ...
      <Button
        ...
        onClick={onContinue}
        disabled={!file}
        className={clsx("w-full mt-4", fill && "flex-shrink-0 mb-[max(1.5rem,env(safe-area-inset-bottom))]")}
      >
        Continue
      </Button>
    </div>
